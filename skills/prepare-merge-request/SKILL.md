...
description: Cleans up a branch before opening a PR/MR. Auto-detects and runs linters with auto-fix, runs the full test suite, checks and fixes inline documentation and docs/ sync, commits any cleanup changes, then creates a draft PR/MR using the default template. Works on GitHub and GitLab.
---

# Prepare Merge Request

Clean up the current branch for review: lint, test, verify documentation, commit any fixes, and create a draft PR/MR. This is the final step before code review.
...
- **If tests FAIL**:
  ...
  - If still failing after 3 attempts, **stop and report the failures**. Do NOT proceed with a PR/MR that has failing tests.
...
- Use platform-appropriate link format (GitHub vs GitLab)
...
## Step 9: Create Draft PR/MR

### Determine the issue reference

Parse the branch name for an issue number (same as update-issue):
...
- **Draft only**: The PR/MR is always created as a draft. The user marks it ready for review after checking.
- **Base branch**: Always target `main`. If `main` doesn't exist, use `master`.
- **Test failures are blockers**: Never create a PR/MR with failing tests. Fix them or stop and report.
